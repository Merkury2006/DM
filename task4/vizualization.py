...
def visualize_graph_with_shortest_path(G, source, target, paths, lengths, filename='result.png'):
    ...
    nx.draw_networkx_edges(G, pos, ax=ax1, edge_color='gray',arrows=True, arrowsize=15, connectionstyle='arc3,rad=0.2')
    ...
    if target in paths:
        ...
        nx.draw_networkx_edges(G, pos, ax=ax2, edge_color='lightgray', arrows=True, arrowsize=15, connectionstyle='arc3,rad=0.2')

        nx.draw_networkx_nodes(G, pos, ax=ax2, nodelist=path_nodes, node_color='orange', node_size=400)
        nx.draw_networkx_edges(G, pos, ax=ax2, edgelist=path_edges,edge_color='red', width=3, arrows=True, arrowsize=20, connectionstyle='arc3,rad=0.2')
